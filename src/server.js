...
async function setFechasProgramadas() {
  ...
  `);
  return new Set(
    (rows || []).map(r => r.fecha_inicio).filter(Boolean).map(f => dayjs(f).format('YYYY-MM-DD'))
  );
}
...
async function calcularProximaFecha(unidadId, preferCedisId = null) {
  ...
  let baseDias = 45;
  if (veces >= 5) baseDias = 35; else if (veces >= 3) baseDias = 40;
  if (ultimo && String(ultimo.tipo || '').toUpperCase() === 'CORRECTIVO') baseDias = Math.min(baseDias, 30);
  ...
  for (let i = 0; i < 180; i++) {
    ...
    if (!esDomingo(iso) && !colisiona7dias(iso, fechasSet)) {
      if (!cedisId) return iso;
...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!rows?.length) return res.render('login', { title: 'Iniciar sesión', error: 'Usuario o contraseña inválidos' });

    const bcrypt = (await import('bcryptjs')).default;
    const ok = await bcrypt.compare(password, rows[0].password_hash);
    if (!ok) return res.render('login', { title: 'Iniciar sesión', error: 'Usuario o contraseña inválidos' });
...
app.get('/', async (req, res, next) => {
  try {
    const mes = (req.query.mes || dayjs().format('YYYY-MM')).substring(0,7);
    const inicio = dayjs(mes + '-01').startOf('month').format('YYYY-MM-DD');
...
app.get('/unidades', async (req, res, next) => {
  try {
    ...
    `;
    const params = [];
    if (cedisId) { sql += ' AND u.cedis_id = ?'; params.push(cedisId); }
